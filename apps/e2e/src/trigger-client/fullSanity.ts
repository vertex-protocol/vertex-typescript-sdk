...
async function fullSanity(context: RunContext) {
  ...
  const clearinghouse = getContract({
    ...
    address: context.contracts.clearinghouse as Address,
    client: walletClient,
  });
  ...
  const endpoint = getContract({
    ...
    address: endpointAddr as Address,
    client: walletClient,
  });
  ...
  await waitForTransaction(
    quote.write.approve([context.contracts.endpoint as Address, depositAmount]),
    publicClient,
  );
  await waitForTransaction(
    depositCollateral({
